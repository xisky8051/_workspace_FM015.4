...
<%--  									src="<c:url value="/assets/images/${ memberIndexs.imgUrl[1] }" />" --%>
<!--  									alt="Card image cap"> -->
<!--                         ============!!!! -->
						...
						<c:forEach items="${ memberIndexs }" var="memberIndex"
							...
							<div class="card"
								...
								<a href="<c:url value="/productdetails/${memberIndex.id}" />"> <img class="card-img-top"
...
<%-- 									<p class="item_number">商品編號：${ memberIndex.orderId }</p> --%>
									<div style="display: flex; justify-content: space-around;">
										<c:if test="${memberIndex.id == 9}">
											<a href="<c:url value='memberindex1/${memberIndex.id}' />"
												class="btn btn-primary">0人登記</a>
											<a href="<c:url value="/productdetails/${memberIndex.id}" />" class="btn btn-success">編輯物品</a>
										</c:if>
										<c:if test="${memberIndex.id == 12}">
											<a href="<c:url value='memberindex1/${memberIndex.id}' />"
												class="btn btn-primary">2人登記</a>
											<a href="<c:url value="/productdetails/${memberIndex.id}" />" class="btn btn-success">編輯物品</a>
										</c:if>
										<c:if test="${memberIndex.id == 16}">
											<a href="<c:url value='memberindex1/${memberIndex.id}' />"
												class="btn btn-primary">8人登記</a>
											<a href="<c:url value="/productdetails/${memberIndex.id}" />" class="btn btn-success">編輯物品</a>
										</c:if>
